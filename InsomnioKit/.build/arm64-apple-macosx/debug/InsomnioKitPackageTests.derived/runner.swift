...
extension SwiftPMXCTestObserver: XCTestObservation {
	var testOutputPath: String {
		"/Users/alfredo/Developer/iOS/Insomnio/InsomnioKit/.build/arm64-apple-macosx/debug/testOutput.txt"
	}

	private func write(record: any Encodable) {
		let lock = FileLock(at: URL(fileURLWithPath: testOutputPath + ".lock"))
		_ = try? lock.withLock {
	...
	private func _write(record: any Encodable) {
		if let data = try? JSONEncoder().encode(record) {
			if let fileHandle = FileHandle(forWritingAtPath: testOutputPath) {
				defer { fileHandle.closeFile() }
				fileHandle.seekToEndOfFile()
				fileHandle.write("\n".data(using: .utf8)!)
				fileHandle.write(data)
			} else {
				_ = try? data.write(to: URL(fileURLWithPath: testOutputPath))
			}
		}
	}
	...
	public func testCase(_ testCase: XCTestCase, didRecord expectedFailure: XCTExpectedFailure) {
		let record = TestCaseFailureRecord(
			testCase: .init(testCase),
			issue: .init(expectedFailure.issue),
			failureKind: .expected(failureReason: expectedFailure.failureReason),
		)
		write(record: TestEventRecord(caseFailure: record))
	}
	#else
	...
	public func testSuite(_ testSuite: XCTestSuite, didRecord expectedFailure: XCTExpectedFailure) {
		let record = TestSuiteFailureRecord(
			suite: .init(testSuite),
			issue: .init(expectedFailure.issue),
			failureKind: .expected(failureReason: expectedFailure.failureReason),
		)
		write(record: TestEventRecord(suiteFailure: record))
...
struct TestCaseFailureRecord: Codable, CustomStringConvertible {
	...
	var description: String {
		"\(issue.sourceCodeContext.description)\(testCase) \(issue.compactDescription)"
	}
}
...
extension TestIssue {
	init(description: String, inFile filePath: String?, atLine lineNumber: Int) {
		let location: TestLocation? = if let filePath {
			.init(file: filePath, line: lineNumber)
		} else {
			nil
		}
		self.init(
			...
			associatedError: nil,
			sourceCodeContext: .init(callStack: [], location: location),
			attachments: [],
		)
	}
}
...
@available(
	*,
	deprecated,
	message: "Not actually deprecated. Marked as deprecated to allow inclusion of deprecated tests (which test deprecated functionality) without warnings"
)
struct Runner {
